...
/**
 * Clean Backtrace Class
 * Filters and formats PHP debug backtraces for better debugging
 */
class WpBackTrace
{
    /**
    ...
    protected static array $fluentClasses = [
        'LaraDumpsCore\Support\WpQuery\\',
    ];

    protected static array $fluentFiles = [
        'wpfluent/framework/src/WPFluent/Database',
    ];
     ...
     *   file?: string,
     * } $frame
     * @return Component:|null An object representing the component, or null if
    ...
    public function get_filtered_trace() {
        ...
        if ( ! isset( $this->filtered_trace ) ) {
            ...
            if ( empty( $trace ) && ! empty( $this->trace ) && false) {
                $lowest = $this->trace[0];
                $file = Util::standard_dir( $lowest['file'], '' );
                $lowest['calling_file'] =$file;
                $lowest['calling_line'] = $file;
    ...
    public function filter_trace( array $frame ) {
        ...
        foreach (self::$fluentClasses as $fluentClass) {
            if(isset($frame['class']) && str_contains($frame['class'],$fluentClass)) {
                return null;
            }
        }

        foreach (self::$fluentFiles as $fluentFile) {
            if(isset($frame['file']) && str_contains($frame['file'],$fluentFile)) {
                return null;
            }
        }
        ...
        if ( $return ) {
            ...
            $return['calling_file'] = $frame['file'] ;
            $return['calling_line'] = $frame['line'] ;
